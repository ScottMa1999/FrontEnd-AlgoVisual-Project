...
/* Dropdown Menu Design Set up */
let drop_down = document.querySelectorAll(".drop-down");

drop_down.forEach(drop => {

  let select = drop.querySelector(".select");
  let caret = drop.querySelector(".caret");
  let menu = drop.querySelector(".menu");
  let options = drop.querySelectorAll(".menu li");
  let selected = drop.querySelector('.selected');


  /* MOUSEOVER the SELECT button */
  select.addEventListener("mouseover", ()=> {
    caret.classList.toggle('caret-colorChanging');
  })

  /* MOUSEOUT the SELECT button */
  select.addEventListener("mouseout",()=>{
    caret.classList.remove('caret-colorChanging');
  })
...
let options = document.getElementById("option-unchanged");
options.addEventListener ("click",()=>{
  selected.innerHTML = "Volume";
})



/* ------ Random bars display set up ------ */
...
speeds.forEach(speed => {
  speed.addEventListener("click", ()=>{
    speedFactor = parseInt(speed.getAttribute('value'));
  })
})

/* ------ Algorithm button set up ------ */
let algotouse = "";

let algos = document.querySelectorAll('#algo');
algos.forEach(algo => {
  algo.addEventListener("click",()=>{
    algotouse = algo.getAttribute('value');
  })
})

/* ------ Randomize button set up ------ */
randomize_array.addEventListener("click", function () {
  unsorted_array = createRandomArray();
  bars_container.innerHTML = "";
  renderBars(unsorted_array);
  randomize_array.backgroundColor = "red";
});

randomize_array.addEventListener("mouseout",()=>{
  randomize_array.style.border = "2px solid black"
  randomize_array.style.transition = "border ease-out 600ms";
})

randomize_array.addEventListener("mouseover",()=>{
  randomize_array.style.border = "6px solid white";
  randomize_array.style.transition = "border ease-out 600ms";
})
...
async function swap(items, leftIndex, rightIndex, bars) {
  ...
  bars[leftIndex].style.backgroundColor = "black";
...
async function mergeSort(arr) {
  ...
  while (i < left.length && j < right.length) {
    ...
    if (k + arr.length < bars.length) {
      ...
      bars[k + arr.length].style.backgroundColor = "black";
    }
    await sleep(speedFactor);
    //bars[k].innerText = arr[k];
...
sort_btn.addEventListener("click", function () {
  ...
  sort_btn.backgroundColor = "red";
});

sort_btn.addEventListener("mouseout",()=>{
  sort_btn.style.border = "2px solid black"
  sort_btn.style.transition = "border ease-out 600ms";
})

sort_btn.addEventListener("mouseover",()=>{
  sort_btn.style.border = "6px solid white";
  sort_btn.style.transition = "border ease-out 600ms";
})

// /* ------ Pause button set up ------ */
// let pause_btn = document.getElementById('pause_button');

// pause_btn.addEventListener("mouseout",()=>{
//   pause_btn.style.border = "2px solid black"
//   pause_btn.style.transition = "border ease-out 600ms";
// })

// pause_btn.addEventListener("mouseover",()=>{
//   pause_btn.style.border = "6px solid white";
//   pause_btn.style.transition = "border ease-out 600ms";
// })
...
let letter = document.querySelector('.typewritter');
let content_all = document.querySelector('.container');

content_all.classList.toggle('hidden');
sub.classList.toggle('hidden');

intro.addEventListener("animationend", ()=>{
  sub.classList.remove('hidden');
  letter.classList.toggle('hidden');
  content_all.classList.remove('hidden');
})
